...
flac = 'Objects\cover.flac'
huge = r"Objects\HUGE_image.png"
...
def image_stega(cover, payload, n_lsb):
    ...
    if data_len > w*h*n_lsb:  # check if the number of bits in payload is more than cover
        ...
        a.save('./client/src/Encoded/encoded.' + cover_filetype)
    ...
    for j in range(height):
        for i in range(width):
            ...
            if i == 0 and j == 0:  # if it is the first pixel of cover image
                ...
                # append subsequent payload bits for red frame into r
                r += payload_bin[0:2]
                # append subsequent payload bits for green frame into g
                g += payload_bin[2:4]
                # append subsequent payload bits into blue frame into b
                b += payload_bin[4:6]
...
def image_decode(stego):
    ...
    if filetype == 'png' or filetype == 'jpg' or filetype == 'bmp':
        ...
        image.save('./client/src/Decoded/decoded.'+filetype)
        print("Payload " + filetype + " reconstructed...")
    else:
        bytemsg = bytearr[:-40]
        temp = bytearray([int(bytemsg[i:i+8], 2)
                          for i in range(0, len(bytemsg), 8)])
        f = open('./client/src/Decoded/decoded.' + filetype, 'wb')
        f.write(temp)
        f.close()
        print("Payload " + filetype + " reconstructed...")
