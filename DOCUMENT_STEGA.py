...
def docu_stega(cover, payload, n_lsb):
    ...
    if data_len > len(docu)*n_lsb:
        print("Cover Document is too small!!!")
    else:
        a = docu_encode(docu, payload_bin, n_lsb)
        with open('./public/images/Encoded/encoded.' + cover_filetype, 'wb') as f:
            f.write(a)
        f.close()
        print("Success")
...
if __name__ == "__main__":
    ...
    if(sys.argv[1] == 'encode'):
        docu_stega(sys.argv[2], sys.argv[3], int(sys.argv[4]))
    elif(sys.argv[1] == 'decode'):
        docu_decode(sys.argv[2])
